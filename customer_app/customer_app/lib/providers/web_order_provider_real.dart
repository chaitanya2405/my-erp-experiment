  ...
  Future<Map<String, dynamic>?> _createOrderWithFullPOSIntegration(Map<String, dynamic> orderData) async {
    try {
      ...
      final adminOrderData = {
        ...
        'store_id': 'STORE_001', // Match admin app's default store ID
  ...
  Future<void> _updateCustomerLoyalty(String customerId, double totalAmount) async {
    try {
      ...
      if (kDebugMode) {
        ...
        print('  • Amount spent: \$${totalAmount.toStringAsFixed(2)}');
        print('  • Points to award: $loyaltyPointsEarned');
      }
      ...
      if (customerQuery.docs.isNotEmpty) {
        ...
        final currentTotalPurchases = currentData['total_purchases'] ?? 0.0;
        final currentTotalOrders = currentData['total_orders'] ?? 0;
        
        final newPoints = currentPoints + loyaltyPointsEarned;
        final newTotalPurchases = currentTotalPurchases + totalAmount;
        ...
        if (kDebugMode) {
          ...
          print('    • Points: $currentPoints → $newPoints (+$loyaltyPointsEarned)');
          print('    • Total orders: $currentTotalOrders → $newTotalOrders');
          ...
          String previousTier = _calculateLoyaltyTier(currentPoints);
          String newTier = _calculateLoyaltyTier(newPoints);
          if (newTier != previousTier) {
            print('    🎉 LOYALTY TIER UPGRADE: $previousTier → $newTier');
          }
        }
      } else {
        if (kDebugMode) {
          print('  ❌ Customer record not found in CRM: $customerId');
        }
      }
    } catch (e) {
      if (kDebugMode) {
        print('❌ Error updating customer CRM record: $e');
      }
    }
  }
  
  // Helper method to calculate loyalty tier
  String _calculateLoyaltyTier(int points) {
