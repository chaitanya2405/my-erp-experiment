...
class _PosAnalyticsScreenState extends State<PosAnalyticsScreen> {
  ...
  void _loadAnalytics() {
    ...
    posProvider.loadAnalytics(_startDate, _endDate);
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (posProvider.error != null)
                  _buildErrorWidget(posProvider.error!)
                else ...[
                  _buildSummaryCards(posProvider.analytics),
                  const SizedBox(height: 20),
                  _buildSalesChart(posProvider.transactions),
                  const SizedBox(height: 20),
                  _buildPaymentModeChart(posProvider.analytics),
                  const SizedBox(height: 20),
                  _buildTopProducts(posProvider),
                  const SizedBox(height: 20),
                  _buildRecentTransactions(posProvider.transactions.take(5).toList()),
                ],
              ],
            ),
          ),
  ...
  Widget _buildTopProducts(PosProvider posProvider) {
    final topProducts = posProvider.getTopSellingProducts(5);
