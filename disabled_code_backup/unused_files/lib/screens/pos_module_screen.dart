...
import '../modules/pos_management/providers/pos_providers.dart';

import '../core/models/unified_models.dart';
import '../models/pos_transaction.dart' as local_models;
...
class _PosModuleScreenState extends ConsumerState<PosModuleScreen> {
  ...
  void initState() {
    ...
    // Initialize POS provider with default store when screen loads
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final posProvider = provider.Provider.of<PosProvider>(context, listen: false);
      if (posProvider.selectedStore.isEmpty) {
        // Set default store ID - you can change this to match your store setup
        posProvider.setSelectedStore('STORE_001');
        print('  ✅ POS Module initialized for Store: STORE_001');
        print('  📊 POS Dashboard ready - monitoring for new transactions...');
        print('  🔄 Integration status:');
        print('    • Inventory sync: ACTIVE');
        print('    • Customer CRM sync: ACTIVE'); 
        print('    • Real-time analytics: ACTIVE');
      } else {
        print('  ✅ POS Module already configured for Store: ${posProvider.selectedStore}');
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final posProvider = provider.Provider.of<PosProvider>(context);
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: posProvider.isOfflineMode ? Colors.red : Colors.green,
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(
                              posProvider.isOfflineMode ? Icons.wifi_off : Icons.wifi,
                              color: Colors.white,
                              size: 16,
                            ),
                            const SizedBox(width: 8),
                            Text(
                              posProvider.isOfflineMode ? 'OFFLINE MODE' : 'ONLINE',
                      ...
                      if (posProvider.offlineTransactions.isNotEmpty) ...[
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              const SizedBox(width: 8),
                              Text(
                                '${posProvider.offlineTransactions.length} pending sync',
                                style: TextStyle(
            ...
            children: [
              Expanded(
                child: _buildQuickSaleStatCard(
                  'Total Sales',
                  posProvider.transactions.where((t) => 
  ...
  Widget _buildRecentTransactionsList() {
    ...
    if (posProvider.transactions.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    final recentTransactions = posProvider.transactions.take(5).toList();
  ...
  Widget _buildTransactionsList() {
    ...
    final transactions = posProvider.transactions;
  ...
  void _navigateToNewTransaction(BuildContext context) async {
    ...
    if (result == true && mounted) {
      ...
      final posProvider = provider.Provider.of<PosProvider>(context, listen: false);
      posProvider.refreshTransactions();
    ...
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => PosTransactionScreen(
          transaction: _convertToLocal(transaction),
        ),
      ),
    );
    ...
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => AddEditPosTransactionScreen(
          transaction: _convertToLegacy(transaction),
        ),
      ),
    );
  ...
  void _exportTransactions(BuildContext context) async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Exported ${posProvider.transactions.length} transactions successfully'),
          backgroundColor: Colors.green,
  ...
  Widget _buildQuickSalesList() {
    ...
    final quickSales = posProvider.transactions.take(3).toList();
  ...
  legacy_models.PosTransaction? _convertToLegacy(UnifiedPOSTransaction? unifiedTransaction) {
    ...
    return legacy_models.PosTransaction(
      transactionId: unifiedTransaction.posTransactionId,
      storeId: unifiedTransaction.storeId ?? '',
      customerId: unifiedTransaction.customerId,
      cashierId: unifiedTransaction.cashierId,
      transactionDate: Timestamp.fromDate(unifiedTransaction.createdAt),
      items: unifiedTransaction.productItems.map((item) => {
        ...
        'tax_amount': item.metadata['taxAmount'] ?? 0.0,
        'mrp': item.metadata['mrp'] ?? 0.0,
        'batch_number': item.metadata['batchNumber'] ?? '',
        'tax_slab': item.metadata['taxSlab'] ?? '18%',
        'category': item.metadata['category'] ?? '',
        'variant': item.metadata['variant'] ?? '',
      }).toList(),
      subtotal: unifiedTransaction.subTotal,
      discountAmount: unifiedTransaction.discountApplied,
      taxAmount: unifiedTransaction.taxAmount,
      totalAmount: unifiedTransaction.totalAmount,
      paymentMethod: unifiedTransaction.paymentMode,
      transactionStatus: 'completed',
      receiptNumber: unifiedTransaction.invoiceNumber,
      customerInfo: unifiedTransaction.metadata['customerInfo'] ?? {},
      notes: unifiedTransaction.metadata['notes'] ?? '',
      createdAt: Timestamp.fromDate(unifiedTransaction.createdAt),
      updatedAt: Timestamp.fromDate(unifiedTransaction.updatedAt),
    );
  }

  // Convert UnifiedPOSTransaction to local PosTransaction  
  local_models.PosTransaction _convertToLocal(UnifiedPOSTransaction unified) {
    return local_models.PosTransaction(
      posTransactionId: unified.posTransactionId,
      storeId: unified.storeId ?? '',
      terminalId: unified.terminalId,
      cashierId: unified.cashierId,
      customerId: unified.customerId,
      transactionTime: unified.createdAt,
      productItems: unified.productItems.map((item) => local_models.PosProductItem(
        productId: item.productId,
        productName: item.productName,
        sku: item.productName, // Use productName as sku fallback
        quantity: item.quantity,
        mrp: item.metadata['mrp']?.toDouble() ?? item.unitPrice,
        sellingPrice: item.unitPrice,
        finalPrice: item.totalPrice / item.quantity,
        batchNumber: item.metadata['batchNumber'] ?? '',
        discountAmount: item.discount ?? 0.0,
        taxAmount: item.metadata['taxAmount']?.toDouble() ?? 0.0,
        taxSlab: item.metadata['taxSlab'] ?? '18%',
      )).toList(),
      pricingEngineSnapshot: {},
      ...
      remarks: unified.metadata['notes']?.toString(),
