...
import '../../customer_order_management/screens/customer_order_detail_screen.dart';
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final orders = snapshot.data!.docs
              .map((doc) => CustomerOrder.fromMap(doc.data() as Map<String, dynamic>))
              .toList();

          // Filtering
          final filteredOrders = orders.where((o) {
            final matchesSearch = _search.isEmpty ||
                (o.orderId ?? '').toLowerCase().contains(_search) ||
                (o.customerId ?? '').toLowerCase().contains(_search) ||
                (o.status ?? '').toLowerCase().contains(_search);
            final matchesStatus = _statusFilter == 'All' || (o.status ?? '') == _statusFilter;
            final matchesDate = _dateRange == null ||
                (o.orderDate.isAfter(_dateRange!.start.subtract(const Duration(days: 1))) &&
                  o.orderDate.isBefore(_dateRange!.end.add(const Duration(days: 1))));
          ...
          final subscriptionOrders = orders.where((o) => (o.metadata['subscription_flag'] ?? false) == true).length;
                                ...
                                SizedBox(
                                  ...
                                  child: BarChart(
                                    BarChartData(
                                      ...
                                      barGroups: [
                                        _barGroup('New', orders.where((o) => (o.status ?? '') == 'New').length, Colors.blue),
                                        _barGroup('Confirmed', orders.where((o) => (o.status ?? '') == 'Confirmed').length, Colors.orange),
                                        _barGroup('Packed', orders.where((o) => (o.status ?? '') == 'Packed').length, Colors.purple),
                                        _barGroup('Out', orders.where((o) => (o.status ?? '') == 'Out for Delivery').length, Colors.teal),
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: PieChart(
                                    PieChartData(
                                      sections: [
                                        _pieSection('UPI', orders.where((o) => (o.metadata['payment_mode'] ?? '') == 'UPI').length, Colors.blue),
                                        _pieSection('Card', orders.where((o) => (o.metadata['payment_mode'] ?? '') == 'Card').length, Colors.orange),
                                        _pieSection('COD', orders.where((o) => (o.metadata['payment_mode'] ?? '') == 'COD').length, Colors.green),
                                        _pieSection('Wallet', orders.where((o) => (o.metadata['payment_mode'] ?? '') == 'Wallet').length, Colors.purple),
                                      ],
                                      sectionsSpace: 2,
  ...
  Future<void> _exportOrdersToCSV(List<CustomerOrder> orders) async {
    ...
    final rows = [
      ...
      ...orders.map((o) => [
        ...
        o.customerId ?? '',
        o.storeId ?? '',
        o.orderDate.toString().split(' ').first,
        o.status ?? '',
        o.paymentStatus ?? '',
        o.metadata['payment_mode'] ?? '',
        o.deliveryStatus ?? '',
        o.metadata['delivery_address']?.toString() ?? '-',
        o.items.map((p) => p.productName).join(', '),
        o.totalAmount,
        o.discount,
        o.taxAmount,
        o.metadata['delivery_charges'] ?? 0.0,
        o.totalAmount,
        (o.metadata['subscription_flag'] ?? false) ? 'Yes' : 'No',
        o.metadata['subscription_plan']?.toString() ?? '-',
        o.metadata['wallet_used']?.toString() ?? '0',
        o.metadata['delivery_slot']?.toString() ?? '-',
        o.metadata['delivery_person_id']?.toString() ?? '-',
        o.metadata['invoice_id']?.toString() ?? '-',
        o.metadata['remarks']?.toString() ?? '-',
        o.createdAt.toString().split(' ').first,
        o.updatedAt.toString().split(' ').first,
      ])
    ];
  ...
  DataRow getRow(int index) {
    ...
    return DataRow(
      cells: [
        ...
        DataCell(Text(o.orderDate.toString().split(' ').first)),
        DataCell(Text(o.status ?? '')),
        DataCell(Text(o.paymentStatus ?? '')),
        DataCell(Text(o.metadata['payment_mode']?.toString() ?? '')),
        DataCell(Text(o.deliveryStatus ?? '')),
        DataCell(Text(o.metadata['delivery_address']?.toString() ?? '-')),
        DataCell(Text(o.items.map((p) => p.productName).join(', '))),
        DataCell(Text(o.totalAmount.toString())),
        DataCell(Text(o.discount.toString())),
        DataCell(Text(o.taxAmount.toString())),
        DataCell(Text((o.metadata['delivery_charges'] ?? 0).toString())),
        DataCell(Text(o.totalAmount.toString())),
        DataCell(Text((o.metadata['subscription_flag'] ?? false) ? 'Yes' : 'No')),
        DataCell(Text(o.metadata['subscription_plan']?.toString() ?? '-')),
        DataCell(Text((o.metadata['wallet_used'] ?? 0).toString())),
        DataCell(Text(o.metadata['delivery_slot']?.toString() ?? '-')),
        DataCell(Text(o.metadata['delivery_person_id']?.toString() ?? '-')),
        DataCell(Text(o.metadata['invoice_id']?.toString() ?? '-')),
        DataCell(Text(o.metadata['remarks']?.toString() ?? '-')),
        DataCell(Text(o.createdAt.toString().split(' ').first)),
        DataCell(Text(o.updatedAt.toString().split(' ').first)),
        DataCell(
          Row(
            children: [
              IconButton(
