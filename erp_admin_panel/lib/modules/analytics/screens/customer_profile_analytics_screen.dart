    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<CustomerProfile>>(
        ...
        builder: (context, snapshot) {
          ...
          final totalLoyalty = customers.fold<int>(0, (sum, c) => sum + (c.loyaltyPoints ?? 0));
          final avgOrderValue = customers.isEmpty
              ? 0.0
              : customers.fold<double>(0, (sum, c) => sum + (c.averageOrderValue ?? 0.0)) / customers.length;
          ...
          final topLoyalty = [...customers]..sort((a, b) => (b.loyaltyPoints ?? 0).compareTo(a.loyaltyPoints ?? 0));
          ...
          final recentFeedback = customers
              ...
              .take(5)
              .map((c) => '${c.fullName ?? 'Unknown'}: ${c.feedbackNotes ?? ''}')
              .toList();
                  ...
                  cells: [
                    ...
                    DataCell(
                      IconButton(
                        ...
                        onPressed: () {
                          // View profile details in dialog
                          showDialog(
                            context: context,
                            builder: (context) => AlertDialog(
                              title: Text('Customer Profile: ${c.fullName}'),
                              content: Column(
                                mainAxisSize: MainAxisSize.min,
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Email: ${c.email}'),
                                  Text('Phone: ${c.mobileNumber}'),
                                  Text('Address: ${c.addressLine1}'),
                                  Text('Date of Birth: ${c.dob?.toString() ?? 'N/A'}'),
                                ],
                              ),
                              actions: [
                                TextButton(
