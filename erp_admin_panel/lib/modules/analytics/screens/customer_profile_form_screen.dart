...
class CustomerProfileFormScreen extends StatefulWidget {
  final UnifiedCustomerProfile? profile;
...
class _CustomerProfileFormScreenState extends State<CustomerProfileFormScreen> {
  ...
  void initState() {
    ...
    loyaltyPointsController = TextEditingController(text: '0'); // loyaltyPoints not in unified model
    totalOrdersController = TextEditingController(text: p?.totalOrders.toString() ?? '0');
    lastOrderDate = p?.lastVisit;
    ...
    feedbackNotesController = TextEditingController(text: p?.feedbackNotes.join(', ') ?? '');
    marketingOptIn = p?.marketingOptIn ?? false;
    firstOrderDateController = TextEditingController(text: ''); // Not in unified model
    lastActivityDateController = TextEditingController(text: ''); // Not in unified model
    churnRiskScoreController = TextEditingController(text: ''); // Not in unified model
    campaignResponsesController = TextEditingController(text: ''); // Not in unified model
    feedbackSentimentController = TextEditingController(text: ''); // Not in unified model
    preferredChannel = 'App'; // Not in unified model
    _selectedCampaigns = [];
  }
              ...
              ElevatedButton(
                onPressed: () async {
                  if (_formKey.currentState?.validate() ?? false) {
                    final profile = UnifiedCustomerProfile(
                      id: widget.profile?.id ?? FirebaseFirestore.instance.collection('customers').doc().id,
                      ...
                      loyaltyTier: widget.profile?.loyaltyTier ?? 'Bronze',
                      preferredStoreId: preferredStoreIdController.text.isEmpty ? null : preferredStoreIdController.text,
                      totalSpent: (double.tryParse(averageOrderValueController.text) ?? 0.0) * (int.tryParse(totalOrdersController.text) ?? 0),
                      ...
                      preferredContactMode: preferredContactMode ?? 'SMS',
                      marketingOptIn: marketingOptIn,
                      referralCode: referralCodeController.text.isEmpty ? null : referralCodeController.text,
                      referredBy: referredByController.text.isEmpty ? null : referredByController.text,
                      supportTickets: widget.profile?.supportTickets ?? [],
                      feedbackNotes: widget.profile?.feedbackNotes ?? [],
