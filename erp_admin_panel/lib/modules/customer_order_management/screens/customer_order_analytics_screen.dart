      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final orders = snapshot.data!.docs
              .map((doc) => UnifiedCustomerOrder.fromMap(doc.data() as Map<String, dynamic>))
              .toList();
          ...
          final subscriptionOrders = orders.where((o) => o.metadata['subscription'] == true).length;
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: PieChart(
                                    PieChartData(
                                      sections: [
                                        _pieSection('UPI', orders.where((o) => o.metadata['paymentMode'] == 'UPI').length, Colors.blue),
                                        _pieSection('Card', orders.where((o) => o.metadata['paymentMode'] == 'Card').length, Colors.orange),
                                        _pieSection('COD', orders.where((o) => o.metadata['paymentMode'] == 'COD').length, Colors.green),
                                        _pieSection('Wallet', orders.where((o) => o.metadata['paymentMode'] == 'Wallet').length, Colors.purple),
                                      ],
                                      sectionsSpace: 2,
  ...
  Future<void> _exportOrdersToCSV(List<CustomerOrder> orders) async {
    ...
    final rows = [
      ...
      ...orders.map((o) => [
        ...
        o.status,
        o.paymentStatus,
        o.metadata['paymentMode'] ?? '-',
        o.metadata['deliveryMode'] ?? '-',
        o.metadata['deliveryAddress']?.toString() ?? '-',
        o.items.map((p) => p.productName).join(', '),
        o.totalAmount,
        o.discount,
        o.taxAmount,
        o.metadata['deliveryCharges'] ?? 0,
        o.totalAmount,
        (o.metadata['subscriptionFlag'] == true) ? 'Yes' : 'No',
        o.metadata['subscriptionPlan'] ?? '-',
        o.metadata['walletUsed'] ?? 0,
        o.metadata['deliverySlot'] ?? '-',
        o.metadata['deliveryPersonId'] ?? '-',
        o.metadata['invoiceId'] ?? '-',
        o.metadata['remarks'] ?? '-',
  ...
  DataRow getRow(int index) {
    ...
    return DataRow(
      cells: [
        ...
        DataCell(Text(o.metadata['paymentMode'] ?? '-')),
        DataCell(Text(o.metadata['deliveryMode'] ?? '-')),
        DataCell(Text(o.metadata['deliveryAddress']?.toString() ?? '-')),
        ...
        DataCell(Text((o.metadata['deliveryCharges'] ?? 0).toString())),
        DataCell(Text(o.totalAmount.toString())),
        DataCell(Text((o.metadata['subscriptionFlag'] == true) ? 'Yes' : 'No')),
        DataCell(Text(o.metadata['subscriptionPlan'] ?? '-')),
        DataCell(Text((o.metadata['walletUsed'] ?? 0).toString())),
        DataCell(Text(o.metadata['deliverySlot'] ?? '-')),
        DataCell(Text(o.metadata['deliveryPersonId'] ?? '-')),
        DataCell(Text(o.metadata['invoiceId'] ?? '-')),
        DataCell(Text(o.metadata['remarks'] ?? '-')),
