...
class _InventoryManagementScreenState extends ConsumerState<InventoryManagementScreen> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ActivityTracker().trackInteraction(
        ...
        data: {
          ...
          'connected_modules': ['pos_management', 'supplier_management', 'product_management'],
          'features_enabled': {
            'real_time_monitoring': true,
            'automatic_alerts': true,
            'cross_store_sync': true,
            'supplier_integration': true,
            'analytics_dashboard': true,
            'mobile_barcode_support': true,
          }
        },
      );
      ...
      // Set up Firestore listeners for detailed inventory notifications
      _setupInventoryChangeListeners();
    });
  }
  
  void _setupInventoryChangeListeners() {
    // Listen to inventory collection changes for detailed notifications
    FirebaseFirestore.instance
        .collection('inventory')
        .snapshots()
        .listen((snapshot) {
      // Track each change with detailed information
      for (var change in snapshot.docChanges) {
        final data = change.doc.data() as Map<String, dynamic>?;
        
        switch (change.type) {
          case DocumentChangeType.added:
            _notifyAllModulesOfNewInventoryItem(change.doc.id, data);
            break;
          case DocumentChangeType.modified:
            _notifyAllModulesOfInventoryUpdate(change.doc.id, data);
            break;
          case DocumentChangeType.removed:
            _notifyAllModulesOfInventoryDeletion(change.doc.id);
            break;
        }
      }
    });
  ...
  void _notifyAllModulesOfNewInventoryItem(String inventoryId, Map<String, dynamic>? inventoryData) {
    ActivityTracker().trackInteraction(
      action: 'new_inventory_notification',
      element: 'all_modules',
      data: {
        'inventory_id': inventoryId,
        'inventory_data': inventoryData,
        'reason': 'new_inventory_item_added_to_firestore',
      },
    );
  }
  
  void _notifyAllModulesOfInventoryUpdate(String inventoryId, Map<String, dynamic>? inventoryData) {
    ActivityTracker().trackInteraction(
      action: 'inventory_update_notification',
      element: 'all_modules',
      data: {
        'inventory_id': inventoryId,
        'inventory_data': inventoryData,
        'reason': 'inventory_item_updated_in_firestore',
      },
    );
  }
  
  void _notifyAllModulesOfInventoryDeletion(String inventoryId) {
    ActivityTracker().trackInteraction(
      action: 'inventory_deletion_notification',
      element: 'all_modules',
      data: {
        'inventory_id': inventoryId,
        'reason': 'inventory_item_deleted_from_firestore',
      },
    );
  }
  
  void _notifyInventoryLevelChange(String inventoryId, int oldLevel, int newLevel, String reason) {
    ActivityTracker().trackInteraction(
      action: 'inventory_level_change',
      element: 'stock_monitoring',
      data: {
        'inventory_id': inventoryId,
        'old_level': oldLevel,
        'new_level': newLevel,
        'change_amount': newLevel - oldLevel,
        'change_type': newLevel > oldLevel ? 'stock_increase' : 'stock_decrease',
        'reason': reason,
        'timestamp': DateTime.now().toIso8601String(),
        'store_context': 'STORE_001',
      },
    );
  }
  
  void _notifyLowStockAlert(String inventoryId, String productName, int currentLevel, int reorderPoint) {
    ActivityTracker().trackInteraction(
      action: 'low_stock_alert',
      element: 'inventory_monitoring',
      data: {
        'inventory_id': inventoryId,
        'product_name': productName,
        'current_level': currentLevel,
        'reorder_point': reorderPoint,
        'severity': currentLevel == 0 ? 'critical' : currentLevel <= reorderPoint * 0.5 ? 'high' : 'medium',
                  ...
                  return Padding(
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        ...
                        // Enhanced Activity Tracking for Inventory Management
                        final menuActions = [
                          'inventory_add_form_access',
                          'inventory_list_view_access', 
                          'inventory_import_tool_access',
                          'inventory_export_tool_access',
                          'inventory_bulk_edit_access',
                          'inventory_archive_tool_access',
                          'inventory_analytics_access',
                          'inventory_duplicate_tool_access',
                        ];
                        
                        final menuDescriptions = [
                          'Adding new inventory items',
                          'Viewing and managing inventory list',
                          'Importing inventory data from files',
                          'Exporting inventory data and reports',
                          'Bulk editing multiple inventory items',
                          'Archive and restore inventory operations',
                          'Inventory analytics and insights dashboard',
                          'Duplicate inventory item creation',
                        ];
                        
                        final menuFeatures = [
                          ['product_info_entry', 'stock_level_setup', 'supplier_assignment', 'reorder_point_config'],
                          ['real_time_stock_levels', 'low_stock_alerts', 'search_filter_sort', 'quick_edit_actions'],
                          ['csv_import', 'excel_import', 'data_validation', 'bulk_stock_updates'],
                          ['inventory_reports', 'stock_valuation', 'movement_history', 'csv_excel_export'],
                          ['multi_select_edit', 'batch_price_updates', 'stock_adjustments', 'supplier_changes'],
                          ['archive_inactive_items', 'restore_archived', 'cleanup_operations', 'data_retention'],
                          ['stock_trends', 'turnover_analysis', 'profitability_metrics', 'forecasting_tools'],
                          ['clone_existing_items', 'batch_variations', 'template_creation', 'quick_setup'],
                        ];
                        
                        ActivityTracker().trackInteraction(
                          action: menuActions[i],
                          element: 'inventory_menu_${i}',
                          data: {
                            'menu_item': _menuTitles[i],
                            'description': menuDescriptions[i],
                            'features_available': menuFeatures[i].length,
                            'timestamp': DateTime.now().toIso8601String(),
                            'store_context': 'STORE_001',
                            'module': 'inventory_management',
                            'capabilities': menuFeatures[i],
                          },
                        );
                      },
                    ),
...
class _ViewInventoryScreenState extends State<_ViewInventoryScreen> {
  ...
  void initState() {
    ...
    // Track ViewInventoryScreen access with enhanced details
    ActivityTracker().trackInteraction(
      action: 'inventory_list_loaded',
      element: 'view_inventory_screen',
      data: {
        'screen_type': 'inventory_list_view',
        'data_source': 'firestore_realtime',
        'features_available': [
          'real_time_updates',
          'search_filtering', 
          'pagination',
  ...
  void _onEdit(InventoryRecord record) {
    // Track inventory edit action
    ActivityTracker().trackInteraction(
      action: 'inventory_edit_initiated',
      element: 'edit_inventory_button',
      data: {
        'inventory_id': record.inventoryId,
        'product_name': record.productName,
        'current_stock': record.quantityAvailable,
        'edit_type': 'record_modification',
        'screen_transition': 'view_to_edit',
  ...
  void _onView(InventoryRecord record) {
    // Track inventory detail view action
    ActivityTracker().trackInteraction(
      action: 'inventory_detail_viewed',
      element: 'view_inventory_detail_button',
      data: {
        'inventory_id': record.inventoryId,
        'product_name': record.productName,
        'stock_level': record.quantityAvailable,
        'view_type': 'detailed_inspection',
        'details_shown': [
          ...
          child: Padding(
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                // Track inventory data loading and analysis
                if (_allDocs.isNotEmpty) {
                  // Calculate some basic analytics for tracking
                  var lowStockCount = 0;
                  ...
                  for (var doc in _allDocs) {
                    ...
                    final stockLevel = (data['stock_level'] as num?)?.toInt() ?? 0;
                    final reorderPoint = (data['reorder_point'] as num?)?.toInt() ?? 0;
                    final costPrice = (data['cost_price'] as num?)?.toDouble() ?? 0.0;
                    final category = data['category'] as String? ?? 'Unknown';
                    
                    if (stockLevel <= reorderPoint) lowStockCount++;
                    totalStockValue += stockLevel * costPrice;
                    categories.add(category);
                  }
                  
                  ActivityTracker().trackInteraction(
                    action: 'inventory_data_loaded',
                    element: 'firestore_inventory_stream',
                    data: {
                      'total_items': _allDocs.length,
                      ...
                      'table_display_config': {
                        ...
                        'filtered_count': _filterDocs(_allDocs).length,
                      },
                      'analytics_summary': {
                        'low_stock_percentage': lowStockCount > 0 ? ((lowStockCount / _allDocs.length) * 100).toStringAsFixed(1) : '0',
                        'stock_health': lowStockCount == 0 ? 'excellent' : lowStockCount < _allDocs.length * 0.1 ? 'good' : 'needs_attention',
                      },
                      'store_context': 'STORE_001',
                      'module': 'inventory_management',
                    },
                  ...
                  // Generate detailed notifications for inventory items (similar to product management)
                  for (var doc in _allDocs.take(5)) { // Show first 5 items for demo
                    final data = doc.data() as Map<String, dynamic>;
                    ActivityTracker().trackInteraction(
                      action: 'inventory_item_processed',
                      element: 'all_modules',
                      data: {
                        'inventory_id': doc.id,
                        'inventory_data': data,
                        'reason': 'inventory_item_loaded_from_firestore',
                      },
                    );
                  }
                }
            ...
            Scrollbar(
              ...
              child: SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  constraints: BoxConstraints(minWidth: 2000, maxWidth: 3200),
    ...
    for (int i = 1; i < _csvData!.length; i++) {
      ...
      try {
        ...
        for (int j = 0; j < headers.length; j++) {
          ...
          if (header == 'last_updated' && value.toString().isNotEmpty) {
            try {
              data[header] = Timestamp.fromDate(DateTime.parse(value.toString()));
            } catch (e) {
              data[header] = Timestamp.now(); // Default to current time if parsing fails
            }
          } else if (header == 'expiry_date' && value.toString().isNotEmpty) {
            try {
              data[header] = Timestamp.fromDate(DateTime.parse(value.toString()));
            } catch (e) {
              data[header] = null; // Allow null for expiry date
            }
          } else if (header == 'manufacture_date' && value.toString().isNotEmpty) {
            try {
              data[header] = Timestamp.fromDate(DateTime.parse(value.toString()));
            } catch (e) {
              data[header] = null; // Allow null for manufacture date
            }
          } else if (['quantity_available', 'quantity_reserved', 'quantity_on_order', 
                     'quantity_damaged', 'quantity_returned', 'reorder_point'].contains(header)) {
            // Convert quantity fields to integers
            data[header] = int.tryParse(value.toString()) ?? 0;
          } else {
            // Handle regular fields including supplier_id and purchase_order_id
            data[header] = value?.toString() ?? '';
          }
        }
        
        // Ensure required fields exist
        if (!data.containsKey('inventory_id') || data['inventory_id'].toString().isEmpty) {
          data['inventory_id'] = 'INV${DateTime.now().millisecondsSinceEpoch}';
        }
...
class _ExportInventoryScreenState extends State<_ExportInventoryScreen> {
  ...
  Future<void> _exportData() async {
    ...
    try {
      ...
      // Define standard headers in a specific order for consistency
      final standardHeaders = [
        'inventory_id', 'product_id', 'product_name', 'product_sku', 'category', 'brand',
        'store_location', 'supplier_id', 'purchase_order_id', 'quantity_available', 
        'quantity_reserved', 'quantity_on_order', 'quantity_damaged', 'quantity_returned',
        'reorder_point', 'batch_number', 'expiry_date', 'manufacture_date', 
        'storage_location', 'entry_mode', 'last_updated', 'stock_status', 'added_by'
      ];
      
      final rows = <List<String>>[standardHeaders];
      for (final doc in snapshot.docs) {
        final data = doc.data();
        final row = standardHeaders.map((header) {
          final value = data[header];
