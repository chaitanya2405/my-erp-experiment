...
class PosProvider with ChangeNotifier {
  ...
  Future<bool> updateTransaction(String transactionId, UnifiedPOSTransaction transaction) async {
    ...
    try {
      if (_isOfflineMode) {
        ...
        await PosService.updateTransaction(transactionId, transaction);
      }
  ...
  Future<bool> syncOfflineTransactions() async {
    ...
    try {
      await PosService.syncOfflineTransactions(_offlineTransactions);
  ...
  List<PosTransaction> filterTransactionsByDateRange(DateTime startDate, DateTime endDate) {
  ...
  List<PosTransaction> filterTransactionsByPaymentMode(String paymentMode) {
    return _transactions.where((transaction) {
  ...
  List<PosTransaction> getTransactionsByCashier(String cashierId) {
    return _transactions.where((transaction) {
  ...
  Future<String?> createIntegratedTransaction(PosTransaction transaction) async {
    _isLoading = true;
    _error = null;
    notifyListeners();
    ...
    try {
      ...
      if (_isOfflineMode) {
        ...
        transactionId = await PosService.createIntegratedTransaction(transaction);
