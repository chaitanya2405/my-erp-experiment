...
class AddEditPosTransactionScreen extends ConsumerStatefulWidget {
  final UnifiedPOSTransaction? transaction;

  const AddEditPosTransactionScreen({Key? key, this.transaction}) : super(key: key);

  @override
  ConsumerState<AddEditPosTransactionScreen> createState() => _AddEditPosTransactionScreenState();
}

class _AddEditPosTransactionScreenState extends ConsumerState<AddEditPosTransactionScreen> {
  ...
  List<UnifiedCustomerProfile> _availableCustomers = [];
  ...
  void _initializeForm() {
    if (widget.transaction != null) {
      ...
      _customerEmailController.text = transaction.customerEmail;
      _selectedPaymentMode = _getValidPaymentMode(transaction.paymentMode);
      _selectedCustomerTier = _getValidCustomerTier(transaction.customerTier);
      _selectedChannel = _getValidChannel(transaction.channel);
      _remarksController.text = transaction.remarks ?? '';
      
      // Populate product items - convert PosProductItem objects to Map format
      _productItems = transaction.productItems.map((item) => {
  ...
  Future<void> _loadAvailableData() async {
    try {
      ...
      setState(() {
        _availableCustomers = results[0] as List<UnifiedCustomerProfile>;
        _availableProducts = results[1] as List<UnifiedProduct>;
        _isLoading = false;
      });
    } catch (e) {
  ...
  void _selectProductForItem(int index, UnifiedProduct product) {
    setState(() {
      _productItems[index] = {
        ...
        'category': _getValidCategoryValue(product.category ?? 'general'),
      ...
      if (index < _itemControllers.length) {
        ...
        _itemControllers[index]['category']?.text = _getValidCategoryValue(product.category ?? 'general');
      }
    });
  ...
  Future<void> _saveTransaction() async {
    ...
    try {
      final posStateNotifier = ref.read(posStateProvider.notifier);
      
      // Generate invoice number if creating new transaction
      String invoiceNumber = widget.transaction?.invoiceNumber ?? '';
      if (invoiceNumber.isEmpty) {
        invoiceNumber = await posStateNotifier.generateInvoiceNumber();
      }

      final transaction = UnifiedPOSTransaction(
        id: widget.transaction?.id ?? '',
        posTransactionId: widget.transaction?.posTransactionId ?? '',
        transactionId: widget.transaction?.transactionId ?? 'TXN_${DateTime.now().millisecondsSinceEpoch}',
        storeId: ref.read(posStateProvider).selectedStore,
        terminalId: 'TERMINAL_001', // Default terminal
        ...
        productItems: _productItems.map((item) => UnifiedPOSTransactionItem(
          productId: item['product_id'] ?? '',
          productName: item['product_name'] ?? '',
          quantity: item['quantity'] ?? 1,
          unitPrice: (item['unit_price'] ?? 0).toDouble(),
          totalPrice: ((item['quantity'] ?? 1) * (item['unit_price'] ?? 0)).toDouble(),
        ...
        subTotal: _subtotal,
        discountApplied: _totalDiscount,
        taxAmount: _gstDetails['total_gst'] ?? 0.0,
      ...
      String? result;
      if (widget.transaction == null) {
        result = await posStateNotifier.createTransaction(transaction);
      } else {
        final success = await posStateNotifier.updateTransaction(transaction);
      ...
      if (result != null) {
        ...
        throw Exception(ref.read(posStateProvider).error ?? 'Failed to save transaction');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
