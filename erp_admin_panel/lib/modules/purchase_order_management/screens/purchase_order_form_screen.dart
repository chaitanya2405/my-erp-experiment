...
import '../../../models/purchase_order.dart' as models;
...
class _PurchaseOrderFormScreenState extends State<PurchaseOrderFormScreen> {
  ...
  Future<void> _initializeData() async {
    try {
      ...
      // Load PO data after suppliers are loaded (needed for validation)
      if (widget.poId != null) {
        await _loadPurchaseOrder(widget.poId!);
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> _fetchProducts() async {
    final snapshot = await FirebaseFirestore.instance
        .collection('products')
        .limit(100) // Limit to first 100 products for better performance
        .get();
  ...
  Future<List<Map<String, dynamic>>> _fetchSuppliers() async {
    final snapshot = await FirebaseFirestore.instance
        ...
        .where('status', isEqualTo: 'Active') // Only load active suppliers
        .limit(50) // Limit to first 50 suppliers
        .get();
    return snapshot.docs.map((doc) {
      final data = doc.data();
      return {
        'id': doc.id,
        'name': data['supplier_name'] ?? data['name'] ?? doc.id,
      };
    }).toList();
  }
  ...
  Future<void> _loadPurchaseOrder(String poId) async {
    ...
    if (doc.exists) {
      ...
      setState(() {
        ...
        _selectedSupplierId = (_suppliers.any((s) => s['id'] == supplierId)) ? supplierId : null;
        ...
        _lineItems = (data['lineItems'] as List<dynamic>? ?? [])
            .map((item) => models.POItem.fromMap(item as Map<String, dynamic>))
            .toList();
      });
    }
  }
        ...
        ? const Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('Loading purchase order data...'),
              ],
            ),
          )
            ...
            child: Form(
              ...
              DropdownButtonFormField<String>(
                ...
                items: _suppliers.isEmpty 
                  ? [const DropdownMenuItem(value: null, child: Text('Loading suppliers...'))]
                  : _suppliers
              ...
              Row(
                children: [
                  ...
                  if (!isView)
                    ElevatedButton(
                      onPressed: () async {
                        final item = await showDialog<models.POItem>(
                          context: context,
