...
class PosService {
  ...
  static Future<String> createTransaction(legacy_models.PosTransaction transaction) async {
    return await createIntegratedTransaction(transaction);
  }

  // Update an existing transaction
  static Future<void> updateTransaction(String transactionId, legacy_models.PosTransaction transaction) async {
    try {
  ...
  static Future<void> syncOfflineTransactions(List<legacy_models.PosTransaction> offlineTransactions) async {
    try {
  ...
  static Future<String> createIntegratedTransaction(legacy_models.PosTransaction transaction) async {
    try {
      if (kDebugMode) {
  ...
  static Future<List<UnifiedCustomerProfile>> searchCustomers(String query) async {
    try {
      final legacyProfiles = await CustomerProfileService.searchCustomers(query);
      return legacyProfiles.map((profile) => _convertToUnifiedCustomerProfile(profile)).toList();
    } catch (e) {
  ...
  static Future<void> _updateInventoryFromSale(legacy_models.PosProductItem item) async {
    try {
  ...
  static Future<void> _updateCustomerProfile(legacy_models.PosTransaction transaction) async {
    try {
  ...
  static Future<void> _createCustomerOrderFromPOS(legacy_models.PosTransaction transaction, String posTransactionId) async {
    try {
      if (kDebugMode) {
      ...
      final customerOrder = UnifiedCustomerOrder(
        ...
        items: transaction.items.map((item) => _convertToUnifiedPOSTransactionItem(item)).toList(),
  ...
  static Future<void> _updateProductSalesStats(List<legacy_models.PosProductItem> items) async {
    try {
  ...
  static Future<void> _updateCustomerLoyaltyPoints(legacy_models.PosTransaction transaction) async {
    try {
      if (transaction.customerId == null) return;
  ...
  // Convert PosProductItem to UnifiedPOSTransactionItem
  static UnifiedPOSTransactionItem _convertToUnifiedPOSTransactionItem(legacy_models.PosProductItem legacyItem) {
    return UnifiedPOSTransactionItem(
      ...
      totalPrice: legacyItem.totalPrice,
      discount: legacyItem.discount,
      metadata: {
        'sku': legacyItem.sku,
        'category': legacyItem.category,
        'tax_amount': legacyItem.taxAmount,
        'tax_slab': legacyItem.taxSlab,
        'batch_number': legacyItem.batchNumber,
        'mrp': legacyItem.mrp,
      },
    );
  }
