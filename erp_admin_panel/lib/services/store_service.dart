...
class StoreService {
  ...
  static Future<List<StorePerformance>> getStorePerformance(
    ...
    try {
      ...
      // Get all data for store, then filter in memory if needed
      final snapshot = await query.get();
      
      // Filter results in memory if date range specified
      List<QueryDocumentSnapshot> filteredDocs = snapshot.docs;
      if (startDate != null || endDate != null) {
        filteredDocs = snapshot.docs.where((doc) {
          final docDate = (doc.data() as Map<String, dynamic>)['date'] as Timestamp?;
          if (docDate == null) return false;
          
          if (startDate != null && docDate.toDate().isBefore(startDate)) return false;
          if (endDate != null && docDate.toDate().isAfter(endDate)) return false;
          
          return true;
        }).toList();
      }
      final performance = filteredDocs.map((doc) => StorePerformance.fromFirestore(doc)).toList();
