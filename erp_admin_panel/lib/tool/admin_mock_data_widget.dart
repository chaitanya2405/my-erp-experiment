...
import 'supplier_auth_setup.dart'; // Add supplier setup import
import '../models/store_models.dart'; // Add store models import
import 'unified_erp_mock_data_generator.dart'; // Add unified generator
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (!_isExpanded && !_loading && !_demoLoading && !_storeLoading && !_unifiedLoading) ...[
              const SizedBox(height: 4),
              Row(
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: _setupDemoCustomer,
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: _generateUnifiedERPData,
                      icon: const Icon(Icons.auto_awesome, size: 16),
                      label: const Text('ERP', style: TextStyle(fontSize: 12)),
                      style: OutlinedButton.styleFrom(
            ...
            // Show loading indicators even when collapsed
            if (_loading || _demoLoading || _storeLoading || _storeDataLoading || _unifiedLoading) ...[
              const SizedBox(height: 12),
              const LinearProgressIndicator(),
              const SizedBox(height: 8),
              Text(
                _loading ? 'Generating mock data...' :
                _demoLoading ? 'Setting up demo customer...' :
                _storeLoading ? 'Generating store data...' :
                _storeDataLoading ? 'Generating comprehensive store data...' :
                _unifiedLoading ? 'Generating unified ERP data...' : '',
                style: const TextStyle(fontSize: 12),
              ),
            ],
            
            // Show results only when expanded
            if (_isExpanded) ...[
              if (_result != null) ...[
                const SizedBox(height: 16),
